...
from tic_tac_toe.Tabla import Tabla, JatekEredmeny, O, X
...
class MegerositesesTanuloRobot(Jatekos):
    """
    ...
    def get_q(self, tabla_hash: int) -> np.ndarray:
        """
        Returns the q values for the state with hash value `tabla_hash`.
        :param tabla_hash: The hash value of the tabla state for which the q values should be returned
        ...
        #
        if tabla_hash in self.q:
            qvals = self.q[tabla_hash]
        else:
            qvals = np.full(9, self.q_init_val)
            self.q[tabla_hash] = qvals

        return qvals

    def get_move(self, tabla: Tabla) -> int:
        """
        Return the next move given the tabla `tabla` based on the current Q values
        :param tabla: The current tabla state
        :return: The next move based on the current Q values for the input state
        """
        tabla_hash = tabla.hash_value()  # type: int
        qvals = self.get_q(tabla_hash)  # type: np.ndarray
        while True:
    ...
    def move(self, tabla: Tabla):
        ...
        """
        m = self.get_move(tabla)
        self.move_history.append((tabla.hash_value(), m))
        _, res, finished = tabla.lepj(m, self.side)
        return res, finished

    def final_result(self, result: JatekEredmeny):
        """
        ...
        """
        if (result == JatekEredmeny.O_NYERT and self.side == O) or (
                result == JatekEredmeny.X_NYERT and self.side == X):
            final_value = WIN_VALUE  # type: float
        elif (result == JatekEredmeny.O_NYERT and self.side == X) or (
                result == JatekEredmeny.X_NYERT and self.side == O):
            final_value = LOSS_VALUE  # type: float
        elif result == JatekEredmeny.DONTETLEN:
